...
import pytest
import yaml
from scenario import Context, State, Relation, Network
...
def test_relation(ctx, n_remote_units):
    ...
    state_in = State(
        leader=True,
        relations=[monitors],
        networks=[Network.default("monitors")])

    with patch("charm.COSProxyCharm._modify_enrichment_file",
               new=MagicMock()) as f:
    ...
    assert known_remote_units == set(f"remote/{i}" for i in range(n_remote_units))
    ...
    with patch.object(
            COSProxyCharm,
            "_modify_enrichment_file",
            wraps=MagicMock) as f3:
