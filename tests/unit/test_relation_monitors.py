...
class TestRelationMonitors(unittest.TestCase):
    ...
    def test_cos_agent(self):
        ...
        cos_agent_scrape_jobs = json.loads(agent_app_data["config"])["metrics_scrape_jobs"]
        prom_scrape_jobs = json.loads(prom_app_data["scrape_jobs"])
        self.assertEqual(prom_scrape_jobs, cos_agent_scrape_jobs)

    def test_cos_agent_without_downstream_prometheus(self):
        ...
        # THEN cos-agent has scrape jobs
        agent_app_data = self.harness.get_relation_data(rel_id_agent, "cos-proxy/0")
        cos_agent_scrape_jobs = json.loads(agent_app_data["config"])["metrics_scrape_jobs"]
        assert cos_agent_scrape_jobs

        # AND WHEN the monitors relation is removed
        self.harness.remove_relation(rel_id_nrpe)
        agent_app_data = self.harness.get_relation_data(rel_id_agent, "cos-proxy/0")
        cos_agent_scrape_jobs = json.loads(agent_app_data["config"])["metrics_scrape_jobs"]
        applications_in_job_labels = [
            s.get("labels", {}).get("juju_application")
            for c in cos_agent_scrape_jobs
            for s in c.get("static_configs", [])
        ]
        # THEN there are no more nrpe jobs
        assert "nrpe" not in applications_in_job_labels
        # AND only the prom-target and vector jobs exist
        assert applications_in_job_labels == ["telegraf", "cos-proxy"]
